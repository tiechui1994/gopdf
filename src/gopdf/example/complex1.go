...
func Complex1Report() {
	...
	h := new(C1Header)
	r.RegisterBand(core.Band(*h), core.Header)
	f := new(C1Footer)
	r.RegisterBand(core.Band(*f), core.Footer)
	s1h := new(C1G1Header)
	r.RegisterGroupBand(core.Band(*s1h), core.GroupHeader, 1)
	s1 := new(C1G1Summary)
	r.RegisterGroupBand(core.Band(*s1), core.GroupSummary, 1)
	s2 := new(C1G2Summary)
	r.RegisterGroupBand(core.Band(*s2), core.GroupSummary, 2)
	r.Records = core.ReadTextFile("invoice.txt", 12)
	//fmt.Printf("Records %v \n", r.Records)
	r.SetPage("A4", "mm", "P")
	r.SetFooterY(265)
	r.Execute("complex1111.pdf")
	r.SaveText("complex1111.txt")
}

type C1Detail struct {
}

func (h C1Detail) GetHeight(report *core.Report) float64 {
	return 6
}
func (h C1Detail) Execute(report *core.Report) {
	cols := report.Records[report.DataPos].([]string)
	y := 2.0
	report.Font("IPAexG", 9, "")
	report.Cell(14, y, cols[5])
	report.Cell(40, y, cols[6])
	hr := core.ParseFloatPanic(cols[7], "")
	report.CellRight(150, y, 20, core.AddComma(strconv.FormatFloat(hr, 'f', 1, 64))+" Hrs")
	amt := core.ParseFloatPanic(cols[8], "")
	report.CellRight(170, y, 26, core.AddComma(strconv.FormatFloat(amt, 'f', 2, 64))+" USD")
	report.SumWork["g1amtcum"] += amt
	report.SumWork["g2amtcum"] += amt
	report.SumWork["g1hrcum"] += hr
	report.SumWork["g2hrcum"] += hr
}
func (h C1Detail) BeforeAddNewPage(report core.Report) int {
	if report.DataPos == 0 {
		//max no
		return 2
	}
...
func (h C1Detail) AfterAddNewPage(report core.Report) int {
...
func (h C1Header) GetHeight(report *core.Report) float64 {
	if report.SumWork["g2item"] == 0.0 {
		return 116
	}
	return 38
}
func (h C1Header) Execute(report *core.Report) {
	cols := report.Records[report.DataPos].([]string)
	y := 32.0
	if report.SumWork["g2item"] == 0.0 {
...
func (h C1G1Summary) GetHeight(report *core.Report) float64 {
	return 7
}
func (h C1G1Summary) Execute(report *core.Report) {
	y := 2.0
	report.LineType("straight", 1)
	report.GrayStroke(0.9)
	...
	report.GrayStroke(0)
	report.Font("MPBOLD", 9, "")
	report.CellRight(150, y, 20, core.AddComma(strconv.FormatFloat(
		report.SumWork["g1hrcum"], 'f', 1, 64))+ " Hrs")
	report.CellRight(170, y, 26, core.AddComma(strconv.FormatFloat(
...
func (h C1G2Summary) GetHeight(report *core.Report) float64 {
	return 50
}
func (h C1G2Summary) Execute(report *core.Report) {
	report.Font("MPBOLD", 9, "")
	y := 15.0
...
func (h C1G1Header) GetHeight(report *core.Report) float64 {
	return 8
}
func (h C1G1Header) Execute(report *core.Report) {
	cols := report.Records[report.DataPos].([]string)
	y := 2.0
	report.Font("MPBOLD", 9, "")
...
func (h C1Footer) GetHeight(report *core.Report) float64 {
	return 10
}
func (h C1Footer) Execute(report *core.Report) {
	report.Cell(100, 12, "Page")
	report.Cell(112, 12, strconv.Itoa(report.GetCurrentPageNo()))
}
