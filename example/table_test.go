...
func handleTable(template Template) (rows, cols int, cells [][]string, hasRowName bool) {
	...
	if hasRowName {
		...
		for i := 1; i < rows-1; i++ {
			for j := 0; j < cols-1; j++ {
				if j == 0 {
					cells[i][j] = body[i-1][j].RowName
				} else {
					switch body[i-1][j].Content.(type) {
					case string:
						cells[i][j] = body[i-1][j].Content.(string)
					case float64:
						cells[i][j] = fmt.Sprintf("%.2f", body[i-1][j].Content)
					}
				}
			}
		}
		...
		for j := 0; j < cols-1; j++ {
			if j == 0 {
				...
				switch template.TableTotalArr[j-1].(type) {
				case string:
					cells[rows-1][j] = template.TableTotalArr[j-1].(string)
				case float64:
					cells[rows-1][j] = fmt.Sprintf("%.2f", template.TableTotalArr[j-1])
				}
			}
		}
	...
	if !hasRowName && !isEmpty(body) {
		for i := 1; i < rows; i++ {
			for j := 0; j < cols; j++ {
				cells[i][j] = body[i][j].Content.(string)
			}
		}
	} else {
		for j := 0; j < cols; j++ {
